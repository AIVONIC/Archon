...
from src.server.services.storage import set_database_backend
from tests.fake_backend import FakeBackend


def _install_summary_backend(sources):
    """Route the summary endpoint at a FakeBackend returning ``sources``.

    The summary service issues a count="exact" query plus per-source batch
    queries; FakeBackend returns the same rows (and count) for each, so the
    rows must carry both ``source_url`` and ``url`` for URL resolution.
    """
    for s in sources:
        s.setdefault("source_url", f"https://example.com/{s['source_id']}")
        s.setdefault("url", s["source_url"])
    set_database_backend(FakeBackend(data=sources, count=len(sources)))
...
def test_knowledge_summary_endpoint(client, mock_supabase_client):
    ...
    _install_summary_backend(mock_sources)
