...
class SourceLinkingService:
    ...
    async def get_project_sources(self, project_id: str) -> tuple[bool, dict[str, list[str]]]:
        ...
        try:
            response = await (
                self.backend.table("archon_project_sources")
    ...
    async def update_project_sources(
        ...
        try:
            ...
            if technical_sources is not None:
                ...
                await self.backend.table("archon_project_sources").delete().eq(
                ...
                for source_id in technical_sources:
                    try:
                        await self.backend.table("archon_project_sources").insert({
                            "project_id": project_id,
                            "source_id": source_id,
                            "notes": "technical",
                        }).execute()
    ...
    async def format_projects_with_sources(
